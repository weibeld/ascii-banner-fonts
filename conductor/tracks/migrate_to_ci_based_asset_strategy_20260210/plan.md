...
- [ ] Task: Remove all currently tracked font assets and legacy extraction scripts.
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Cleanup' (Protocol in workflow.md)

## Phase 2: Pipeline Implementation
- [ ] Task: Create `versions.json` (Version Manifest) pinning FIGlet and patorjk collection.
- [ ] Task: Implement `scripts/fetch-patorjk-fonts.js` using `npm pack`.
- [ ] Task: Implement `scripts/generate-fonts-manifest.js` (targets `src/generated/fonts.json`).
- [ ] Task: Update `package.json` with lifecycle scripts (`setup`, `dev`, `build`).
...
- [ ] Task: Implement `.github/workflows/sync_versions.yml` (Version Maintenance Bot).
