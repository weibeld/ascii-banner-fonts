# Implementation Plan - Project Refinement & Roadmap Planning

## Phase 1: Knowledge Extraction
- [x] Task: Extract atomic items from `brain-dump-2026-02-20.md` to `items/` verbatim.
- [x] Task: Organise items into logical groups via file renaming.
- [x] Task: Review and verify extraction and grouping integrity.
- [x] Task: Conductor - User Manual Verification 'Phase 1: Extraction' (Protocol in workflow.md)

## Phase 2: Strategic Alignment
- [x] Task: Evaluate "Central Font Repository" architecture.
- [x] Task: Establish "Launch Pad" for Font Data backend (`font_data_backend.md`).

## Phase 3: Item-by-Item Grooming

### Architectural Strategy
- [x] Task: Process `ARCH_STRATEGY_02_backend_data_strategy.md`.

### Font Curation
- [x] Groom: `FONT_CURATION_01_exclusions.md`
- [x] Groom: `FONT_CURATION_02_fixes.md`
- [x] Groom: `FONT_CURATION_03_renaming.md`
- [x] Groom: `FONT_CURATION_04_whitespace.md`
- [x] Groom: `FONT_CURATION_05_line_endings.md`

### Font Metadata
- [x] Groom: `FONT_METADATA_01_supported_chars.md`
- [x] Groom: `FONT_METADATA_02_error_messages.md`
- [x] Groom: `FONT_METADATA_03_ascii_vs_ansi.md`
- [x] Groom: `FONT_METADATA_04_bug_unknown_categories.md`
- [ ] Groom: `FONT_METADATA_05_patorjk_types.md`
- [ ] Groom: `FONT_METADATA_06_flf_specs.md`
- [ ] Groom: `FONT_METADATA_07_toilet_category.md`
- [ ] Groom: `FONT_METADATA_08_lowercase_only.md`
- [ ] Groom: `FONT_METADATA_09_bug_size_categorisation.md`

### Font Sources
- [ ] Groom: `FONT_SOURCES_01_figlet_org.md`
- [ ] Groom: `FONT_SOURCES_02_xero_analysis.md`
- [ ] Groom: `FONT_SOURCES_03_xero_integration.md`
- [ ] Groom: `FONT_SOURCES_04_toilet_fonts.md`

### Infrastructure & Pipeline
- [ ] Groom: `INFRA_PIPELINE_01_plugin_system.md`
- [ ] Groom: `INFRA_PIPELINE_02_renovate_bot.md`

### UI & UX
- [ ] Groom: `UI_UX_01_tweaks.md`
- [ ] Groom: `UI_UX_02_dynamic_card_lines.md`
- [ ] Groom: `UI_UX_03_per_card_editing.md`
- [ ] Groom: `UI_UX_04_favourite_system.md`
- [ ] Groom: `UI_UX_05_code_comment_option.md`
- [ ] Groom: `UI_UX_06_display_header_comments.md`
- [ ] Groom: `UI_UX_07_glitches.md`
- [ ] Groom: `UI_UX_08_rendering_errors.md`

### Research
- [ ] Groom: `RESEARCH_01_competitor_categorisation.md`
- [ ] Groom: `RESEARCH_02_ascii_arts_web_ring.md`
- [ ] Groom: `RESEARCH_03_arial_ascii.md`
- [ ] Groom: `RESEARCH_04_overlapping_chars.md`

## Phase 4: Finalisation
- [ ] Task: Update project `todo.md` with remaining minor chores.
- [ ] Task: Archive this track.
